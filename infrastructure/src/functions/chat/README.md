...
| 変数名              | 必須    | デフォルト値     | 説明                                       |
| :------------------ | :------ | :--------------- | :----------------------------------------- |
| TABLE_NAME          | **Yes** | \-               | チャット履歴を保存するDynamoDBテーブル名   |
| PINECONE_INDEX_NAME | No      | documents        | ベクトル検索を行うPineconeのインデックス名 |
| STAGE               | No      | local            | 環境（local, dev, prod）                   |
| USER_POOL_ID        | Cond    | \-               | Cognito User Pool ID（本番認証時に必須）   |
| CLIENT_ID           | Cond    | \-               | Cognito Client ID（本番認証時に必須）      |
| MODEL_ID            | No      | (Claude 3 Haiku) | 使用するBedrockのモデルID                  |
...
| error        | { "errorCode": "INTERNAL_SERVER_ERROR", "message": "..." }              | エラー発生時。                                                 |
